...
export const WorkoutMemberSelector = ({ selectedMember, onSelect, onView, members }) => {
  return (
    <div className="p-4 w-full mx-auto font-figtree">
      <header className="flex flex-col gap-1 pb-4">
        <h2 className="text-2xl font-bold text-slate-900">Quản lý bài tập</h2>
        <p className="text-slate-600">Chọn hội viên để quản lý bài tập</p>
      </header>
     <Card className="p-6 flex flex-row justify-between items-center gap-4 rounded-2xl shadow-sm border-slate-100">
        <div className="w-[200px] min-w-[200px] max-w-[200px] shrink-0">
          ...
          <Select value={selectedMember} onValueChange={onSelect}>
            <SelectTrigger className="h-11 w-[200px] min-w-[200px] max-w-[200px] rounded-xl">
              <SelectValue placeholder="Chọn hội viên..." className="truncate" />
            </SelectTrigger>
            <SelectContent>
              {members.map(m => (
